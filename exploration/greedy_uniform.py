...
def get_greedy_uniform_exploration_action(ob_np, policy=None, qfs=None, sample_size=32):
    ...
    # begin = torch.tanh(pre_tanh_mu_T-2*std)
    # end = torch.tanh(pre_tanh_mu_T+2*std)
    # actions = Uniform(begin, end).sample([sample_size])

    begin = pre_tanh_mu_T-2*std
    end = pre_tanh_mu_T+2*std
